...
void functionTestCommat(){
	...
	processCommats(strings, numberCharacter, numberOfLines);
...
void checkCommats(char* string, int sizeLine, int numberLignes){
	int y = 0;
	const char toCheck = ',';

	for(y = 0; y < sizeLine; y++){
		if(y == (sizeLine - 1) && string[y] == toCheck){
			fprintf(stdout, "Syntaxe error on line with operator : %c at line : %d on character number : %d\n", toCheck, numberLignes + 1, y + 1);
		}else{
...
void processCommats(char** strings, int* sizeLine, int numberLignes){
	int i = 0;

	for(i = 0; i < numberLignes; i++){
		checkCommats(strings[i], sizeLine[i], i);
	}	
}